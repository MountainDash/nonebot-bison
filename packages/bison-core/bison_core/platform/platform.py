...
from bison_core.typing import Target
from bison_core.utils.classproperty import classproperty
...
if TYPE_CHECKING:
    ...
    from bison_core.site import ProcessContext, SiteConfig
    from bison_core.typing import Category, SubUnit, Tag
...
class FilterMixin[RawPost](ABC):
    ...
    @abstractmethod
    async def filter(self, target: Target, raw_posts: Iterable[RawPost]) -> tuple[RawPost, ...]:
        """过滤消息, 返回符合订阅条件的消息"""
        ...
...
class ParseTargetMixin(ABC):
    @property
    def parse_target_promot(self) -> str | None:
        """解析平台账号时的提示语"""
        return None
    ...
    async def get_target_name(
        cls,
        client: HTTPClientMixin,
        target: Target,
    ) -> str | None: ...

    class ParseTargetException(PlatformException):
