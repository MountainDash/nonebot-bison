...
def get_config_theme(platform: PlatformConfig, default_theme: str) -> str | None:
    """获取用户指定的theme"""
    return platform.preferred_theme or default_theme

def get_priority_themes(platform: PlatformConfig) -> list[str]:
    """获取渲染所使用的theme名列表，按照优先级排序"""
    themes_by_priority: list[str] = []
    # 最先使用用户指定的theme
    if user_theme := get_config_theme(platform, DEFAULT_THEME):
