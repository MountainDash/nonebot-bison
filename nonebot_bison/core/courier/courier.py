...
from .param import (
    ...
    MetadataParam,
    MetaFetchParam,
    NonebotTaskGroupParam,
    ParcelParam,
    PayloadParam,
    ReceiptParam,
)
from .parcel import Parcel
from .types import ChannelConflictException, DeliveryReceiverHandle, Road
...
if TYPE_CHECKING:
    ...
    from .types import Address, ChannelName, DeliveryReceiver, Roadmaps
...
class Courier:
    ...
    def __post_init__(self):
        ...
        self.running_channels: dict[ChannelName, Channel[Any]] = {}
    ...
    def receive_from(
        ...
        def wrapper[R](
            receiver: DeliveryReceiverHandle[R],
        ) -> DeliveryReceiverHandle[R]:
            dependentable = Dependent[Parcel[R] | None].parse(
                call=receiver,
                parameterless=parameterless,
                allow_types=cls.PARCEL_PARAM_TYPES,
            )
    ...
    async def post(self, parcel: Parcel[Any]):
        ...
        if parcel.tag not in self.roadmaps:
            await self._recycle_dead_letter(
                parcel, reason=f"No roadmap found for address {parcel.tag!r}"
            )
            return
        ...
        return await self.running_channels[road.channel].post(parcel)

    def receive_from_dead_letter(
        self,
        receiver: DeliveryReceiverHandle[Any],
        parameterless: Iterable[Any] | None = None,
        ...
        self.dead_letter_receiver = Dependent[Parcel[Any] | None].parse(
            call=receiver,
            parameterless=parameterless,
            allow_types=self.PARCEL_PARAM_TYPES,
        )
        return receiver

    async def run_channel(self, tg: TaskGroup, channel: ChannelName, *, override: bool = False):
        async def _run_parcel_forwarding(channel: ChannelName):
            running_channel = self.running_channels[channel]

            async for parcel in running_channel.delivery():
                if self.close_event.is_set():
                    logger.info("Courier is closing, stopping parcel forwarding")
                    break
                ...
                if not (send_road := self.roadmaps.get(address)):
                    ...
                    await self._recycle_dead_letter(
                        parcel, reason=f"No roadmap found for address {address!r}"
                    )
                    continue
                if send_road.channel != channel:
                    logger.warning(
                        f"Roadmap channel mismatch for parcel {parcel!s}, recycling"
                    )
                    await self._recycle_dead_letter(
                        parcel,
                        reason=f"Roadmap channel mismatch: {send_road.channel!r} != {channel!r}",
                    )
                    continue
                ...
                try:
                    match await send_road.receiver(parcel=parcel):
                        ...
                        case Parcel(tag) if tag == parcel.tag:
                            ...
                            await self._recycle_dead_letter(
                                parcel,
                                reason=f"Receiver returned a parcel with the same tag: {tag!r}",
                            )
                        ...
                        case other:
                            logger.error(
                                f"Receiver returned an unexpected value: {other!r}"
                            )
                            await self._recycle_dead_letter(
                                parcel, reason=f"Unexpected return value: {other!r}"
                            )
                    ...
                    await self._recycle_dead_letter(
                        parcel,
                        reason=f"dead by these exception:\n{'\n'.join(dead_reasons)}",
                        exc=e,
                    )
                else:
                    logger.debug(
                        f"Parcel {parcel!s} processed successfully by receiver {send_road.receiver}"
                    )
                finally:
                    continue

        maybe_running_channel = self.running_channels.get(channel)
        if not override and maybe_running_channel is not None:
            logger.critical(f"Channel {channel!r} is already running")
            raise ChannelConflictException(f"Channel {channel!r} is already running")

        self.running_channels[channel] = self._create_channel(channel)

        if maybe_running_channel is not None:
            maybe_running_channel.close()
    ...
    async def run(self):
        ...
        for channel in self.plan_channels:
            await self.run_channel(self.task_group, channel)

        self.task_group.start_soon(_run_dead_letter_receiver)

    def close(self):
        """关闭"""
        for channel in self.running_channels.values():
            channel.close()
    ...
    async def _recycle_dead_letter(
        ...
        if not dead_letter.receipt.is_dead:
            ...
            logger.warning(
                f"Dead letter {dead_letter!s} was already dead, but received again"
            )
            return
    ...
    def _create_channel(self, name: str) -> Channel:
        entrance, outlet = create_memory_object_stream(
            max_buffer_size=self.channel_buffer_size
        )
        return Channel(
            name=name,
            conveyor=Conveyor(entrance, outlet),
            dead_letter_entrance=(
                self.dead_letter_channel.entrance if self.dead_letter_channel else None
            ),
