...
MESSAGE_STREAM_ENTRANCE, MESSAGE_STREAM_OUTLET = anyio.create_memory_object_stream[MessageQueueItem](
    plugin_config.max_concurrency.send_message
)

MESSGE_SEND_INTERVAL = plugin_config.max_concurrency.resend_wait
...
async def _send_msgs_dispatch(send_target: PlatformTarget, msg: Sendable):
    if plugin_config.send_use_queue:
        await MESSAGE_STREAM_ENTRANCE.send(MessageQueueItem(send_target, msg, plugin_config.send_resend_times))
    else:
...
async def send_msgs(send_target: PlatformTarget, msgs: list[MessageFactory]):
    if not plugin_config.send_use_pic_merge:
        ...
        return
    msgs = msgs.copy()
    if plugin_config.send_use_pic_merge == 1:
