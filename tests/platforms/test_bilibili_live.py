...
async def test_fetch_first_live(bili_live, dummy_only_open_user_subinfo):
    ...
    assert post2.card_data.platform == "Bilibili直播"
    assert post2.card_data.text == "[开播] 【Zc】从0挑战到15肉鸽！目前10难度"
    assert post2.card_data.url == "https://live.bilibili.com/3044248"
    assert post2.card_data.target_name == "魔法Zc目录 其他单机"
...
async def test_fetch_bililive_only_title_change(bili_live, dummy_only_title_user_subinfo):
    ...
    assert post3.card_data.platform == "Bilibili直播"
    assert post3.card_data.text == "[标题更新] 【Zc】从0挑战到15肉鸽！目前12难度"
    assert post3.card_data.url == "https://live.bilibili.com/3044248"
    assert post3.card_data.target_name == "魔法Zc目录 其他单机"
...
async def test_fetch_bililive_only_close(bili_live, dummy_only_close_user_subinfo):
    ...
    assert isinstance(post4.card_data, PlainStem)
    assert post4.card_data.platform == "Bilibili直播"
    assert post4.card_data.text == "[下播] 【Zc】从0挑战到15肉鸽！目前12难度"
    assert post4.card_data.url == "https://live.bilibili.com/3044248"
    assert post4.card_data.target_name == "魔法Zc目录 其他单机"
...
async def test_fetch_bililive_combo(bili_live, dummy_bililive_user_subinfo):
    ...
    assert isinstance(post2.card_data, PlainStem)
    assert post2.card_data.platform == "Bilibili直播"
    assert post2.card_data.text == "[开播] 【Zc】从0挑战到15肉鸽！目前11难度"
    assert post2.card_data.url == "https://live.bilibili.com/3044248"
    assert post2.card_data.target_name == "魔法Zc目录 其他单机"
    assert post2.pics == ["https://i0.hdslb.com/bfs/live/new_room_cover/fd357f0f3cbbb48e9acfbcda616b946c2454c56c.jpg"]
    assert post2.compress is True
