...
from nonebot_bison.db.db_config import SubscribeDupException, data_access
from nonebot_bison.db.db_model import Cookie as DBCookie
from nonebot_bison.db.subs_io.utils import NBESFParseErr
...
async def subs_receipt_gen(nbesf_data: SubGroup):
    ...
    for item in nbesf_data.groups:
        ...
        for sub in item.subs:
            ...
            try:
                await data_access.add_subscribe(receipt.user, **model_dump(receipt, exclude={"user"}))
...
async def magic_cookie_gen(nbesf_data: SubGroup):
    ...
    for cookie in nbesf_data.cookies:
        try:
            ...
            cookie_id = await data_access.add_cookie(new_cookie)
            for target in cookie.targets:
                await data_access.add_cookie_target(T_Target(target.target), target.platform_name, cookie_id)
        except Exception as e:
