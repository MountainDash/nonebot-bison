...
@pytest.mark.asyncio
async def test_get_tag_without_topic_info(bilibili: "Bilibili", bing_dy_list):
    simple_raw_post = {
        "display": {
            "topic_info": {
                "topic_details": [
...
@pytest.mark.asyncio
async def test_video_forward(bilibili: "Bilibili", bing_dy_list):
    post = await bilibili.plain_parse(bing_dy_list[1])
    assert (
        post.card_data.text  # type: ignore
...
@pytest.mark.asyncio
async def test_video_forward_without_dynamic(bilibili: "Bilibili", bing_dy_list):
    # 视频简介和动态文本其中一方为空的情况
    post = await bilibili.plain_parse(bing_dy_list[2])
    assert (
        post.card_data.text  # type: ignore
...
async def test_article_forward(bilibili: "Bilibili", bing_dy_list):
    post = await bilibili.plain_parse(bing_dy_list[4])
    assert (
        post.card_data.text  # type: ignore
        == "#明日方舟##饼学大厦#\n9.11专栏更新完毕，"
        "这还塌了实属没跟新运营对上\n后边除了周日发饼和PV没提及的中文语音，"
...
@pytest.mark.asyncio
async def test_dynamic_forward(bilibili: "Bilibili", bing_dy_list):
    post = await bilibili.plain_parse(bing_dy_list[5])
    assert (
        post.card_data.text  # type: ignore
        == "饼组主线饼学预测——9.11版\n①今日结果\n9.11 殿堂上的游禽-星极(x，"
...
@pytest.mark.asyncio
@respx.mock
async def test_fetch_new_without_dynamic(bilibili: "Bilibili", dummy_user_subinfo, without_dynamic):
...
@pytest.mark.asyncio
@respx.mock
async def test_fetch_new(bilibili: "Bilibili", dummy_user_subinfo):
