...
from bison_core.platform import (
    CompletedPlatform,
    is_batch_platform,
    platform,
)
from bison_core.site import (
    ClientManager,
    ProcessContext,
    SiteConfig,
...
from bison_core.typing import SubUnit, Target, UserSubInfo
...
class Scheduler(ABC):
    ...
    @abstractmethod
    async def get_current_weight_val(self) -> dict[str, int]: ...

    async def get_next_schedulable(self) -> Schedulable | None:
        if not self.schedulable_list:
            return None
        cur_weight = await self.get_current_weight_val()
        ...
        logger.debug(f"next schedulable is {cur_max_schedulable=}")
        return cur_max_schedulable

    @abstractmethod
    async def get_userinfo(self, platform_name: str, target: Target) -> list[UserSubInfo]: ...

    @abstractmethod
    async def process_fetch_result(self, sub_units: list[SubUnit], platform: CompletedPlatform): ...
    ...
    async def exec_fetch(self):
        ...
        platform_obj = platform[schedulable.platform_name].bound(context)
        try:
            if schedulable.use_batch:
                ...
                for batch_target in batch_targets:
                    userinfo = await self.get_userinfo(schedulable.platform_name, batch_target)
                    sub_units.append(SubUnit(batch_target, userinfo))
                await self.process_fetch_result(sub_units, platform_obj)
            else:
                send_userinfo_list = await self.get_userinfo(schedulable.platform_name, schedulable.target)
                await self.process_fetch_result([SubUnit(schedulable.target, send_userinfo_list)], platform_obj)
    ...
    def insert_new_schedulable(self, platform_name: str, target: Target):
        ...
        new_schedulable = Schedulable(
            ...
            target,
            1000,
            is_batch_platform(platform[platform_name]()),
        )
    ...
    def delete_schedulable(self, platform_name, target: Target):
        if is_batch_platform(platform[platform_name]()):
    ...
    def __str__(self) -> str:
        return f"<Scheduler {self.name} | {self.scheduler_config} | {self.schedulable_list}>"
