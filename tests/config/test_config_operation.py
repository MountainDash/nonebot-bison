...
async def test_add_subscribe(app: App, init_scheduler):
    ...
    assert len(confs) == 1
    conf1: Subscribe = confs[0]
    async with AsyncSession(get_engine()) as sess:
        related_user_obj = await sess.scalar(select(User).where(User.id == conf1.user_id))
        related_target_obj = await sess.scalar(select(Target).where(Target.id == conf1.target_id))
    ...
    assert conf1.target.target == "weibo_id"
    assert conf1.categories == []
