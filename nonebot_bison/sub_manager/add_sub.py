...
from nonebot_plugin_alconna import on_alconna, Extension, Alconna, CommandMeta
import contextlib
...
add_sub_alc = Alconna(
    "添加订阅",
    meta=CommandMeta(
        description="Bison 添加订阅指令",
        usage="输入“添加订阅”，跟随 Bot 提示信息进行"
    ),
)
...
async def add_sub_handler(
    ...
    async for platform in check_platform(timeout=600, retry=5, prompt=""):
        ...
        if platform == "全部":
            await add_sub_command.send(
                "全部平台\n" + "\n".join(
                    [f"{platform_name}: {platform.name}" for platform_name,
                     platform in platform_manager.items()]
                ))
            continue
        elif platform == "取消":
    ...
    if cur_platform.categories:
        ...
        await add_sub_command.send(
            "请输入要订阅的类别，以空格分隔，支持的类别有：{}".format(
                " ".join(list(cur_platform.categories.values()))
            )
        )
