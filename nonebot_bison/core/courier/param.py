...
class NonebotTaskGroupParam(Param):
    """Nonebot 任务组注入参数

    本注入解析所有类型为且仅为`anyio.abc.TaskGroup`的参数。
    """

    def __repr__(self) -> str:
        return "NonebotTaskGroupParam()"

    @classmethod
    @override
    def _check_param(cls, param: inspect.Parameter, allow_types: tuple[type[Param], ...]) -> Self | None:
        if generic_check_issubclass(param.annotation, TaskGroup):
            return cls()

    @override
    async def _solve(self, task_group: TaskGroup, **kwargs: Any) -> Any:
        return task_group
